...
with no_grad():
  ...
  latents = randn(latents_shape, generator=generator, device='cpu', dtype=text_embeddings.dtype).to(device)

  alphas_cumprod: Tensor = get_alphas_cumprod(get_alphas(get_betas(device=device)))
